...
> **Note:** the actual workflow in this PR (`.github/workflows/run-pipeline.yml`)
> is `workflow_dispatch`-triggered rather than folded into `test.yml`'s
> `pull_request` trigger as §4 below describes, so it doesn't gate PR
> checks on hardware that doesn't exist yet. Everything else here (the
> `runs-on` label, host sizing, runner setup steps) still applies as-is.
> Once a runner is confirmed online and a run has actually succeeded, §4's
> original plan of moving `integration-test` itself onto it becomes the
> natural follow-up.
